OptimizeAdditiveDecompositionModel=function(Data,SeasonalLength=52,Horizon=12,Iterations=100,PlotIt=TRUE){
  ...
  opt=function(x){
    ...
    beta=x[2]
    gamma=x[3]
    y=AdditiveDecompositionModel(Train,SeasonalLength,0,alpha,beta,gamma,Silent=TRUE)
    if(length(y)>1)
  ...
  for(i in 1:Iterations){
    ...
    fit=AdditiveDecompositionModel(Train,SeasonalLength = SeasonalLength,Horizon,outtmp$p1,outtmp$p2,outtmp$p3,Silent=TRUE)
  ...
  print(c(out$p1,out$p2,out$p3))
  fit=AdditiveDecompositionModel(Train,SeasonalLength = SeasonalLength,Horizon,out$p1,out$p2,out$p3)
